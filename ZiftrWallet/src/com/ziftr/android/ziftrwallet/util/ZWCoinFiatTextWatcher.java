...
public class ZWCoinFiatTextWatcher implements TextWatcher {
	...
	public ZWCoinFiatTextWatcher(OWCoin coin, EditText coinEdit, EditText fiatEdit) {
		this.coin = coin;
		...
		if(s == coinEdit.getEditableText()) {
			...
			if(userEdited) {
				try {
					...
					BigDecimal fiatValue = OWConverter.convert(coinValue, coin, selectedFiat);
			...
			if(userEdited) {
				try {
					...
					BigDecimal coinValue = OWConverter.convert(fiatValue, selectedFiat, coin);
