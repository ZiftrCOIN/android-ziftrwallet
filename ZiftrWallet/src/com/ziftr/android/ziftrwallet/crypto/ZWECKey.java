...
public class ZWECKey {
	 ...
	 */
	private ZWKeyCrypter keyCrypter;

	/**
	 * The encrypted private key information.
	 */
	private ZWEncryptedData encryptedPrivateKey;
	 ...
	 */
	public static ZWECKey fromASN1(byte[] asn1privkey) {
		return new ZWECKey(extractPrivateKeyFromASN1(asn1privkey));
	}

	/** Creates an ECKey given the private key only.  The public key is calculated from it (this is slow) */
	public ZWECKey(BigInteger privKey) {
		this(privKey, (byte[])null);
	}

	/** A constructor variant with BigInteger pubkey. See {@link ZWECKey#ECKey(BigInteger, byte[])}. */
	public ZWECKey(BigInteger privKey, BigInteger pubKey) {
	 ...
	 */
	public ZWECKey(@Nullable byte[] privKeyBytes, @Nullable byte[] pubKey) {
	 ...
	 */
	public ZWECKey(@Nullable ZWEncryptedData encryptedPrivateKey, @Nullable byte[] pubKey, ZWKeyCrypter keyCrypter) {
	 ...
	 */
	public ZWECKey(@Nullable BigInteger privKey, @Nullable byte[] pubKey, boolean compressed) {
	 ...
	 * be used for signing.
	 */
	private ZWECKey(@Nullable BigInteger privKey, @Nullable byte[] pubKey) {
	...
	/**
	 * converts the string to a {@link ZWSha256Hash} then signs it and returns the R and S components as BigIntegers. In the Bitcoin protocol, they are
	...
	public boolean verify(byte[] data, byte[] signature) {
		return ZWECKey.verify(data, signature, getPubKey());
	}
	...
	public boolean verify(ZWSha256Hash sigHash, ZWECDSASignature signature) {
		return ZWECKey.verify(sigHash.getBytes(), signature, getPubKey());
	}
	...
	public String signMessage(ZWCoin coin, String message) throws ZWKeyCrypterException {
		...
		int recId = -1;
		for (int i = 0; i < 4; i++) {
			ZWECKey k = ZWECKey.recoverFromSignature(i, sig, hash, isCompressed());
	 ...
	 */
	public static ZWECKey signedMessageToKey(byte[] messageBytes, String signatureBase64) throws SignatureException {
		...
		int recId = header - 27;
		ZWECKey key = ZWECKey.recoverFromSignature(recId, sig, messageHash, compressed);
		if (key == null)
	...
	/**
	 * Convenience wrapper around {@link ZWECKey#signedMessageToKey(String, String)}. If the key derived from the
	...
	public void verifyMessage(byte[] messageBytes, String signatureBase64) throws SignatureException {
		ZWECKey key = ZWECKey.signedMessageToKey(messageBytes, signatureBase64);
	 ...
	 */
	@Nullable
	public static ZWECKey recoverFromSignature(int recId, ZWECDSASignature sig, ZWSha256Hash message, boolean compressed) {
		...
		return new ZWECKey((byte[])null, q.getEncoded(compressed));
	}
	 ...
	 * @return encryptedKey
	 */
	public ZWECKey encrypt(ZWKeyCrypter keyCrypter) throws ZWKeyCrypterException {
		// Will throw an error if cannot decrypt using the key crypter field
		ZWECKey decrypted = this;
		if (this.isEncrypted()) {
		...
		ZWECKey result = new ZWECKey(encryptedPrivateKey, getPubKey(), keyCrypter);
		result.setCreationTimeSeconds(creationTimeSeconds);
		return result;
	}

	public ZWECKey encrypt() throws ZWKeyCrypterException {
	 ...
	 */
	public static boolean encryptionIsReversible(ZWECKey originalKey, ZWECKey encryptedKey) {
		String genericErrorText = "The check that encryption could be reversed failed for key " + originalKey.toString() + ". ";
		try {
			ZWECKey rebornUnencryptedKey = encryptedKey.decrypt();
	...
	public void setKeyCrypter(ZWKeyCrypter keyCrypter) {
		this.keyCrypter = keyCrypter;
		if (this.keyCrypter != null) {
			ZWECKey encrypted = this.encrypt();
